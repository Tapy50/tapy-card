                        ...
                        <div class="card-body">
                            ...
                            <div class="text-gray-900 fw-bolder fs-2 mb-2 mt-5">
                                @if(Auth::guard('web')->user()->type == 'company'  )
                                    <?php
                                    $employees = \App\Models\User::where('company_id',Auth::guard('web')->id())->pluck('id');
                                    ?>
                                    {{\App\Models\AppointmentsRelation::whereIn('user_id',$employees)->OrWhere('user_id',Auth::guard('web')->id())->count()}}
                                @else
                    ...
                    <a href="#" class="card bg-dark hoverable card-xl-stretch mb-xl-8">
                        ...
                        <div class="card-body">
                            ...
                            <div class="text-gray-100 fw-bolder fs-2 mb-2 mt-5">
                                @if(Auth::guard('web')->user()->type == 'company'  )
                                    {{\App\Models\Card::where('company_id',Auth::guard('web')->id())->OrWhere('user_id',Auth::guard('web')->id())->count()}}
                                @else
                            ...
                            <div class="text-white fw-bolder fs-2 mb-2 mt-5">
                                @if(Auth::guard('web')->user()->type == 'company'  )
                                    <?php
                                        $employees = \App\Models\User::where('company_id',Auth::guard('web')->id())->pluck('id');
                                    ?>
                                    {{\App\Models\ExchangeContact::whereIn('user_id',$employees)->OrWhere('user_id',Auth::guard('web')->id())->count()}}
                                @else
                        ...
                        <div class="card-body">
                            ...
                            <div class="text-white fw-bolder fs-2 mb-2 mt-5">
                                @if(Auth::guard('web')->user()->type == 'company'  )
                                    ...
                                    {{\App\Models\Profile::where('user_id',Auth::guard('web')->id())->OrWhere('company_id',Auth::guard('web')->id())->count()}}
                                @else
            ...
            @if(Auth::guard('web')->check())
                <div class="col-md-12">
                    <div class="card card-xl-stretch mb-xl-8">
                        ...
                        <div class="card-header border-0 pt-5">
                            <h3 class="card-title align-items-start flex-column">
                                <span class="card-label fw-bolder fs-3 mb-1">{{__('lang.Tapping')}}</span>
                            </h3>
                            <!--begin::Toolbar-->
                            <div class="card-toolbar" data-kt-buttons="true">
                            </div>
    ...
    <script type="text/javascript">
        $(function () {
            var table = $('#users_table').DataTable({
                ...
                ajax: {
                    url: '{{ route('Tapping.datatable.data') }}',
                    data: {
                    }
                },
                columns: [
                    {data: 'checkbox', name: 'checkbox', "searchable": false, "orderable": false},
                    {data: 'os', name: 'os', "searchable": true, "orderable": true},
                    {data: 'ip', name: 'ip', "searchable": true, "orderable": true},
                    {data: 'browser', name: 'browser', "searchable": true, "orderable": true},
                ...
                series: [
                    {
                        ...
                        data: [
                            @if(Auth::guard('web')->user()->type == 'company'  )
                                ...
                                @for($x=1; $x<=12; $x++)
                                "{{$uses->whereIn('user_id',$employees)->OrWhere('user_id',Auth::guard('web')->id())->whereYear('created_at',date('Y'))->where('states','new')->whereMonth('created_at','=',$x)->count()}}",
                                  @endfor
                                @else

                                @inject('uses','App\Models\AppointmentsRelation')
                                @for($x=1; $x<=12; $x++)
                                "{{$uses->where('employee_id',Auth::guard('web')->id())->whereYear('created_at',date('Y'))->where('states','new')->whereMonth('created_at','=',$x)->count()}}",
                                 @endfor
                            @endif
                    ...
                    },
                    {
                        ...
                        data: [
                            ...
                            <?php
                                ...
                                @inject('uses','App\Models\AppointmentsRelation')
                                @for($x=1; $x<=12; $x++)
                                "{{$uses->whereIn('user_id',$employees)->OrWhere('user_id',Auth::guard('web')->id())->whereYear('created_at',date('Y'))->where('states','complate')->whereMonth('created_at','=',$x)->count()}}",
                            @endfor
                                @else

                                @inject('uses','App\Models\AppointmentsRelation')
                                @for($x=1; $x<=12; $x++)
                                "{{$uses->where('employee_id',Auth::guard('web')->id())->whereYear('created_at',date('Y'))->where('states','complate')->whereMonth('created_at','=',$x)->count()}}",
                            @endfor
                                @endif
